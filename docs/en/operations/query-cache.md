...
[query_cache_max_size](settings/settings.md#query-cache-max-size) and [query_cache_max_entries](settings/settings.md#query-cache-size-max-items)
in a user profile in `users.xml`, then make both settings readonly:
